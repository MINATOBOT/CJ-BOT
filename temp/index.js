	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `YAE SEJA BEM VINDO MEUS COMANDOS .menu @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}*`
				...
				teks = `JA SAIU TARDE @${num.split('@')[0]} kkkk troxas 👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*CJ BOT* : ${me.name}\n*+1 (239) 977-4688* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total de block* : ${blocked.length}\n*CJ.BOT ESTA VIVO DESDE* : ${kyun(uptime)}`
					buffer = await getBuffer(me.imgUrl)
					client.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
				case 'listablock':
					teks = 'numeros que o bot deu block' :\n'
				...
				case 'meme':
					...
					break
case 'oi':
		if (args.length < 1) return reply ('fala ai ?')
		break
				case '(dono)':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQMY-61hXaROz_yIjqISOf5MknLzjcNCpMosg&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*NoME BRUNO\Wa.me/+5573988667150*'})
					break	
				/*case 'memeindo':
					memein = await kagApi.memeindo()
